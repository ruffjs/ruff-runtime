...
The HTTP interfaces in ruff are designed to support many features
...
In order to support the full spectrum of possible HTTP applications, ruff's
...
And ruff does not check whether Content-Length and the length of the body
...
`response.write()` is called, ruff assumes you're going to be streaming
...
Ruff maintains several connections per server to make HTTP requests.
...
* Sending a 'Connection: keep-alive' will notify ruff that the connection to
...
socket to become free the socket is closed. This means that ruff's
...
marked so as to not keep the ruff process running.  However, it is
...
Note: ruff does not check whether Content-Length and the length of the body
which has been transmitted are equal or not.
...
For efficiency reasons, ruff normally buffers the request headers until you
...
`require('url').parse(request.url)`.  Example:
    ...
    ruff> require('url').parse('/status?name=ryan')
...
`true` as the second argument to `require('url').parse`.  Example:
    ...
    ruff> require('url').parse('/status?name=ryan', true)
    { href: '/status?name=ryan',
      search: '?name=ryan',
      query: { name: 'ryan' },
      pathname: '/status' }
