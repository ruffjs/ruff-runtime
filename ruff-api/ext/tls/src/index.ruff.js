...
TLSSocket.prototype._tlsWrite = function (data, encoding, cb) {
    var that = this;
    var packetSize = 1024 * 16;
    var count = parseInt(data.length / packetSize);
    var restDatLength = data.length % packetSize;
    var writeBuf;
    var writeLen = 0;
    ...
    function next () {
        ...
        if (index >= count) {
            if (restDatLength > 0) {
                packetData = data.slice(index * packetSize);
                writeBuf = util._toNativeBuffer(packetData);
                writeLen += writeBuf.length;
                mbedtls.write(that._tlsHandle, writeBuf);
            }
        ...
        packetData = data.slice(index * packetSize, (index + 1) * packetSize);
        writeBuf = util._toNativeBuffer(packetData);
        writeLen += writeBuf.length;
        mbedtls.write(that._tlsHandle, writeBuf);
        index ++;
