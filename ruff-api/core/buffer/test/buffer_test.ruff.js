...
exports['test should ensure unique value is always copied'] = function () {
    var buffer = Buffer(1024); // safe constructor
...
// copy 512 bytes, from 0 to 512.
    ...
    var copied = buffer.copy(buffer2, 0, 0, 512);
    assert.strictEqual(512, copied);
    for (var i = 0; i < buffer2.length; i++) {
        assert.strictEqual(buffer[i], buffer2[i]);
    }

// copy buffer2 into buffer, without specifying sourceEnd
    buffer.fill(++position);
    buffer2.fill(++position);
    var copied = buffer2.copy(buffer, 0, 0);
    assert.strictEqual(buffer2.length, copied);
    for (var i = 0; i < buffer2.length; i++) {
        assert.strictEqual(buffer2[i], buffer[i]);
    }

// copy buffer2 into buffer, without specifying sourceStart
    buffer.fill(++position);
    buffer2.fill(++position);
    var copied = buffer2.copy(buffer, 0);
    assert.strictEqual(buffer2.length, copied);
    for (var i = 0; i < buffer2.length; i++) {
        assert.strictEqual(buffer2[i], buffer[i]);
    }

// copy longer buffer buffer to shorter buffer2 without targetStart
    buffer.fill(++position);
    buffer2.fill(++position);
    var copied = buffer.copy(buffer2);
    assert.strictEqual(buffer2.length, copied);
    for (var i = 0; i < buffer2.length; i++) {
        assert.strictEqual(buffer[i], buffer2[i]);
    }

// copy starting near end of buffer to buffer2
    buffer.fill(++position);
    buffer2.fill(++position);
    var copied = buffer.copy(buffer2, 0, buffer.length - Math.floor(buffer2.length / 2));
    assert.strictEqual(Math.floor(buffer2.length / 2), copied);
    for (var i = 0; i < Math.floor(buffer2.length / 2); i++) {
        assert.strictEqual(buffer[buffer.length - Math.floor(buffer2.length / 2) + i], buffer2[i]);
    }
    for (var i = Math.floor(buffer2.length / 2) + 1; i < buffer2.length; i++) {
        assert.strictEqual(buffer2[buffer2.length - 1], buffer2[i]);
    }

// try to copy 513 bytes, and check we don't overrun buffer2
    buffer.fill(++position);
    buffer2.fill(++position);
    var copied = buffer.copy(buffer2, 0, 0, 513);
    assert.strictEqual(buffer2.length, copied);
    for (var i = 0; i < buffer2.length; i++) {
        assert.strictEqual(buffer[i], buffer2[i]);
    }

// copy 768 bytes from buffer into buffer
    buffer.fill(++position);
    buffer.fill(++position, 256);
    var copied = buffer.copy(buffer, 0, 256, 1024);
    assert.strictEqual(768, copied);
    for (var i = 0; i < buffer.length; i++) {
        assert.strictEqual(position, buffer[i]);
    }
...
exports['test should copy'] = function () {
    var position = 0;
    var buffer1 = Buffer(1024); // safe constructor
    var buffer2 = new Buffer(512);
...
exports['test buffer indexOf'] = function() {
    require('./test-buffer-indexof.js');
}
