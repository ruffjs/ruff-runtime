...
function RuffBox(ruffBoxFilePath) {
    ...
    this._objectMap = Object.create(null);
    if (data.objects instanceof Array) {
        for (var i = 0; i < data.objects.length; i++) {
            var info = data.objects[i];
            ...
            var possiblePath = path.join(path.dirname(this.path), 'ruff_modules', module);
            var ctor = require(possiblePath);
            this._objectMap[ctor.id] = new ctor(options);
        }
    }
}
...
RuffBox.prototype.getDevice = function (deviceId, callback) {
    ...
    if (!info) {
        callback(new Error('Unknown device ID "' + deviceId + '"'));
        return;
    }
    ...
    var DeviceConstructor = require(info.driver);
    ...
    if (info.outputs) {
        context.features = Object.create(null);
        var outputDevices = Object.keys(info.outputs);
        for (var i = 0; i < outputDevices.length; i++) {
            var outputDevice = outputDevices[i];
            context.features[outputDevice] = info.outputs[outputDevice].features;
        }
    }
    ...
    eachSeries(inputNames, function (name, next) {
        ...
        // specific devices, call interface driver getInterface and set default args
        ...
        if (DeviceConstructor.async) {
            ...
            try {
                ...
                console.log('Failed to load driver "' + info.driver + '" for device "' + deviceId + '"');
                callback(e);
                return;
            }
            attachDevice(device);
...
RuffBox.prototype.query = function (query, options, callback) {
    ...
    if (typeof query !== 'string' || query[0] !== '#') {
        throw new Error('Invalid device query, currently only queries in the form of "#<device-id>" are supported');
    }
    */
    ...
    if (query[0] !== '#') {
        var error;
        var object = that._objectMap[query];
        if (!object) {
            error = new Error('System object "' + query + '" not found!');
        }
        if (callback) {
...
function eachSeries(values, handler, callback) {
    ...
    function next(error) {
        ...
        setImmediate(handler, values.shift(), next);
    }
}
