...
// Workaround: Add an active handle in UV,
// this will be removed in the future
setTimeout(function () {
    console.log('1000000s timeout');
}, 1000 * 1000 * 1000);
...
function RuffBox(ruffBoxFilePath) {
    ...
    if (data.objects instanceof Array) {
        for (var i = 0; i < data.objects.length; i++) {
            ...
            var module_dirname = global.ruff ? 'ruff_modules' : 'node_modules';
            var possiblePath = path.join(path.dirname(this.path), module_dirname, module);
...
RuffBox.prototype.getDevice = function (deviceId, callback) {
    ...
    // NOTE: test framework will set `_getDeviceConstructor`
    if (this._getDeviceConstructor) {
        DeviceConstructor = this._getDeviceConstructor(info.driver);
    } else {
        var module_dirname = global.ruff ? 'ruff_modules' : 'node_modules';
        var possibleDriverPath = path.join(path.dirname(this.path), module_dirname, info.driver);
