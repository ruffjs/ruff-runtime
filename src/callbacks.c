...
void duv_read_cb(uv_stream_t *handle, ssize_t nread, const uv_buf_t *buf) {
    ...
    if (nread == UV_EOF) {
        ...
        duk_push_undefined(ctx); // data is undefined
    }
...
void duv_udp_recv_cb(uv_udp_t *handle, ssize_t nread, const uv_buf_t *buf, const struct sockaddr *addr,
                     unsigned flags) {
    duk_context *ctx = handle->loop->data;
    ...
    if (nread >= 0) {
        char *out;
        duk_push_null(ctx);
        duk_push_int(ctx, nread);
        out = duk_push_fixed_buffer(ctx, nread);
        memcpy(out, buf->base, nread);
        duv_push_sockaddr(ctx, (struct sockaddr_storage *)addr);
        free(buf->base);
    }
    ...
    duv_emit_event(ctx, handle->data, DUV_RECV, 4);
}
