...
//deprecated APIs
#if 0
static void duv_fs_event_cb(uv_fs_event_t *req, const char* path, int events, int status) {
    duk_context *ctx = req->loop->data;
    duk_push_uint(ctx, events);
    duk_push_uint(ctx, status);
    duv_fulfill_req(ctx, (uv_req_t*)req, 2);
}
duk_ret_t duv_fs_event_watch(duk_context *ctx) {
    const char* path = duk_require_string(ctx, 0);
    duv_req_t* data;
    uv_fs_event_t *fs_event_req = duk_push_fixed_buffer(ctx, sizeof(*fs_event_req));
    uv_fs_event_init(duv_loop(ctx), fs_event_req);
    fs_event_req->data = duv_setup_req(ctx, 1);
    data = fs_event_req->data;
    duv_check(ctx, uv_fs_event_start(fs_event_req,
                                     duv_fs_event_cb,
                                     path,
                                     UV_FS_EVENT_RECURSIVE));
//    duv_req_t *data = fs_event_req->data;
//    duv_push_ref(ctx, data->req_ref);
    return 1;
}

duk_ret_t duv_fs_stop_event_watch(duk_context *ctx) {
    duk_size_t size;
    uv_fs_event_t *fs_event_req = duk_require_buffer(ctx, 0, &size);

    if (size != sizeof(uv_fs_event_t)) {
        fprintf(stderr, "invalid watch handle\n");
        return DUK_RET_TYPE_ERROR;
    }
    duv_check(ctx, uv_fs_event_stop(fs_event_req));
    fs_event_req->data = duv_cleanup_req(ctx, fs_event_req->data);
    uv_close((uv_handle_t*) fs_event_req, NULL);
    return 0;
}
#endif
...
duk_ret_t duv_fs_read(duk_context *ctx) {
  ...
  int64_t offset = duk_get_int_default(ctx, 2, -1);
  uv_buf_t buf;
  uv_fs_t* req;
...
duk_ret_t duv_fs_write(duk_context *ctx) {
  ...
  offset = duk_get_int_default(ctx, 2, -1);
